...
module ActionView
  module Helpers # :nodoc:
    ...
    module UrlHelper
      ...
      #          <strong>Email me:</strong> <span>me@domain.com</span>
      #        </a>
      def mail_to(recipients, name = nil,  html_options = {}, cc: nil, bcc: nil, reply_to: nil, subject: nil, body: nil, **kwrest, &block)
        html_options, name = name, nil if name.is_a?(Hash)
        html_options = (html_options || {}).merge(kwrest).stringify_keys

        recipients = Array(recipients).map { |c| ERB::Util.html_escape(c) }
        extras = []
        extras << ["cc",       Array(cc).map { |c| ERB::Util.url_encode(c) }.join(",")      ] if cc.present?
        extras << ["bcc",      Array(bcc).map { |c| ERB::Util.url_encode(c) }.join(",")     ] if bcc.present?
        extras << ["body",     ERB::Util.url_encode(body)                                   ] if body.present?
        extras << ["subject",  ERB::Util.url_encode(subject)                                ] if subject.present?
        extras << ["reply-to", Array(reply_to).map { |c| ERB::Util.url_encode(c) }.join(",")] if reply_to.present?
        uri = URI::MailTo.build(to: recipients, headers: extras)

        html_options["href"] = uri.to_s
        content_tag("a", name || recipients.join(", "), html_options, &block)
      end
